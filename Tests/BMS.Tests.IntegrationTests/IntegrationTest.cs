...
public class IntegrationTest
{
    ...
    private readonly int _signalRWaitingTimeoutInSeconds = 40;
    ...
    private async Task<string> RegisterTestAccountAsync(string email)
    {
        ...
        var result = await _signalR.WaitForSignalREventWithConditionAsync(_signalRWaitingTimeoutInSeconds, messages =>
            messages.Any(m => m.RequestId == customerRegistrationInfo.RequestId));
    ...
    public async Task TestSuccessWithdraw()
    {
        ...
        Assert.True(_signalR.Messages.First(e => e.RequestId == accountTransactionInfo.RequestId).IsSuccessful);
